...
executable_name="Contents/MacOS/Supermarket Simulator"
...
case ${os_type} in
    Darwin*)
        ...
        if ! echo "$real_executable_name" | grep "^.*/Contents/MacOS/.*"; then
    ...
    ;;
    *)
        # Supermarket Simulator runs through Proton on Linux, so tell the user they don't need to run this script
        echo "If you're trying to use BepInEx with Supermarket Simulator on Linux, you don't need to use this script!"
        echo "Please refer back to the instructions for proper usage."
        exit 1
        ;;
esac
